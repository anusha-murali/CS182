...
##        Q_values = [sum([prob*(reward + self.gamma * self.V[s_])
##                             for prob, s_, reward, _ in self.env.P[state][action]]) 
##                                   for action in range(self.env.action_space.n)]

        Q_values = [sum([prob*(self.gamma * ( reward + self.V[s_]))
                             for prob, s_, reward, _ in self.env.P[state][action]]) 
                                   for action in range(self.env.action_space.n)]
    ...
    def value_iteration(self):
        ...
        Q_table = np.copy(self.V)

        i = 0
        
        # Update Q-values until convergence (i.e: the difference between 2 consecutive
        # iteration becomes <= epsilon
        while True:
            # Backup the previous Q values for checking convergence
            prevQ_table = np.copy(self.V) 
             
            # For each state, compute the Q-values over all the actions and update it with the
            # maximum Q value
            for s in range(self.env.observation_space.n):
                # Compute the Q values for all the actions from state s
                Q_values = self.updated_action_values(s)
                    
                # Update the Q_table[s] with the MAX of Q values of all actions
                Q_table[s] = max(Q_values)

            self.V = np.copy(Q_table)
            ...
            if (np.sum(np.fabs(prevQ_table - Q_table)) <= self.epsilon):
##                print("BFEAKING ...")
                print(self.V)
                print("i = ", i)
                break
...
##            Q_values = [sum([prob*(r + gamma * Q_table[s_])
##                             for prob, s_, r, _ in env.P[s][a]]) 
##                                   for a in range(env.action_space.n)] 
                
            #extract policy by selecting the action which has maximum Q value
            Q_values = self.updated_action_values(s)
            self.policy[s] = np.argmax(np.array(Q_values)) 
##        print("ME = ", Q_table == self.V)
##        print("after updating policy = ", self.policy)
##        
